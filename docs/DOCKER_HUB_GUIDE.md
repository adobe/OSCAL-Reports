...
1. From the repository root, run the build-and-push script. It reads the version from `package.json` and tags the image as `v<VERSION>` (e.g. `v1.7.20`), then pushes that tag and also `latest`:
...
The image name and tag format match the GitHub workflow (e.g. `keekar/oscal_reports:v1.7.20`), so pull commands for users stay the same. The CI workflow (on tag push or manual trigger) can still be used when you want to publish from GitHub.
...
#### Issue: Some Tags Work, Others Don't (e.g. 1.7 / 1.7.8 work; v1.7.20 / latest don't)
