...
### Added
- **Role Management UI**:
  - Added dropdown selector in User Management page to change user roles
  - Platform Admins can now update user roles directly from the GUI
  - Roles can be changed between User, Assessor, and Platform Admin
  - Confirmation dialog before role changes to prevent accidental modifications
  - Real-time role updates with visual feedback
  - Styled dropdown with color-coded role indicators (Green for User, Yellow for Assessor, Red/Black for Admin)

### Changed
- **User Management Interface**:
  - Replaced static role badges with interactive dropdowns
  - Enhanced user experience with hover effects and visual transitions
  - Improved role change workflow with clear confirmation messages
