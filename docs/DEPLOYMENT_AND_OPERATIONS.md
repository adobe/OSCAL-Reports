...
# OSCAL Report Generator — Deployment & Operations Guide

**Last updated:** 2026-09-08
**Maintainer:** Mukesh Kesharwani <mkesharw@adobe.com>
**Version:** see [CHANGELOG.md](CHANGELOG.md)

Consolidated guide covering how to deploy the OSCAL Report Generator (local, Docker,
TrueNAS Blue/Green, and AWS EC2) and how to operate it on AWS (Terraform layout,
Image Factory AMIs, Bedrock IAM/setup, EC2 Blue/Green hosting practices, and costs).

> This document merges the former `DEPLOYMENT.md`, `CLOUD_DEPLOYMENT.md`, and
> `AWS_OPERATIONS.md`. Retired multi-cloud paths (Azure, Google Cloud Run, ECS/EKS,
> Heroku, DigitalOcean, and the manual single-VM SSH+Docker+Nginx flow) have been
> dropped — production is AWS Terraform Blue/Green (SSM-only, no SSH), with TrueNAS
> Blue/Green as the on-premises option.
...
10. [AWS Terraform for OSCAL (AI via Bedrock)](#aws-terraform-for-oscal-ai-via-bedrock)
11. [Adobe Image Factory – AMI usage for Terraform](#adobe-image-factory-ami-usage-for-terraform)
12. [Amazon Bedrock Integration – Step-by-Step AWS Setup](#amazon-bedrock-integration-step-by-step-aws-setup)
13. [EC2 Web Hosting Best Practices](#ec2-web-hosting-best-practices)
14. [💰 AWS EC2 Cost Estimate for OSCAL Report Generator + Ollama](#aws-ec2-cost-estimate-for-oscal-report-generator-ollama)

---

## Deployment Options Comparison

Three supported deployment paths. Everything else (Azure, Google Cloud Run,
ECS/EKS, Heroku, DigitalOcean, manual single-VM Docker) has been retired.

| Option | Cost | Setup | Best for |
|--------|------|-------|----------|
| **AWS EC2 (Terraform, Blue/Green)** | AWS usage | ~30 min | Production (AMS / AWS4403). See [AWS Terraform for OSCAL](#aws-terraform-for-oscal-ai-via-bedrock). |
| **TrueNAS (Blue/Green, Docker Hub)** | Free (own hardware) | ~20 min | On-premises / lab. See [TrueNAS Deployment](#truenas-deployment). |
| **Docker images only** | Free | Automatic via CI/CD | Custom hosting / air-gapped. See [DOCKER_HUB_GUIDE.md](DOCKER_HUB_GUIDE.md). |

**Local development** runs the full stack on your laptop — see [Local Development](#local-development).

GitHub Actions automatically builds and publishes Docker images to the container
registry on pushes to `Main`; see [DOCKER_HUB_GUIDE.md](DOCKER_HUB_GUIDE.md) and the
...
- **Node.js** 24+ (for local development; see `engines` in `package.json`)
...
- **Canonical GitHub repo** (no VPN required on TrueNAS)
...
# ✓ Current version: 1.7.27 (example — use values printed by the script)
# ✓ GitHub version: 1.7.27
...
1. **Upload** a fresh app tree to **`s3://<environment-logs-bucket>/installer/`** (excludes `.cursor`, `terraform/`, large dev folders). From the repo root: `./scripts/deploy-to-ec2.sh --update-s3` (AWS credentials resolved on the operator laptop via `pass`, same pattern as `terraform/run-with-aws-pass.sh`; `pass` is a laptop-only deploy helper and is never used on the instances themselves).
...
**References:** [AWS Terraform for OSCAL](#aws-terraform-for-oscal-ai-via-bedrock) below, [terraform/README.md](../terraform/README.md), header comments in `scripts/deploy-to-ec2.sh` and `scripts/ec2_automation.sh`.
...
- **Local / Docker:** Set `OSCAL_CONFIG_FIELD_SECRET` or `SESSION_SECRET`. Docker entrypoint auto-generates `/data/.field-secret` and `/data/.session-secret`. Run `node backend/scripts/migrate-config-to-cfgenc.mjs` to encrypt legacy plaintext/`_pass`. `pass` is **not** a runtime dependency — it is an optional laptop-only helper for operator Secrets Manager sync.
- **EC2 production:** AWS SM only; GUI save fails if SM unavailable; startup auto-migrates plaintext to SM. Legacy `_pass` pointers are left untouched at runtime and must be migrated with the operator CLI (`migrate-config-to-cfgenc.mjs` / `migrate-config-to-sm.mjs`).

### Environment Variables

For non-secret configuration (e.g. JWT), use environment variables:
...
- **Validation System**: See [QUALITY_ASSURANCE.md](QUALITY_ASSURANCE.md) — *Part 4: Validation System*
- **Git & release**: See [GIT_AND_RELEASE.md](GIT_AND_RELEASE.md#single-repository-setup-guide)
- **Version History**: See [CHANGELOG.md](CHANGELOG.md)

---

<!-- ===== AWS OPERATIONS (merged from former docs/AWS_OPERATIONS.md) ===== -->
...
If SSO/Slack settings were lost and config.default is stale, run on the instance: `node scripts/debug/repair-ec2-config-from-backups.mjs`, then **re-publish** config.default.
...
**Corporate PKI (Adobe PLM):** CSR, private key, issued cert storage, PLM portal, cutover history, and renewal notes are documented in [BEST_PRACTICES.md](BEST_PRACTICES.md#tls-certificates-and-corporate-pki).
...
- [Deployment Options Comparison](#deployment-options-comparison) – local, Docker, TrueNAS, and AWS EC2 paths
...
| SSAAU-169 / SSAAU-216 / InfraSec | Dynamic EMR lookup + `check-ami-drift.sh`; staggered ASG refresh to latest IF EMR (e.g. **3.0.2**). See [CHANGELOG.md](CHANGELOG.md#1725---2026-07-18). |
| SSAAU-212 / Splunk SCC | `oscal_splunk_uf_bootstrap_enabled` (default true); SSM post-boot re-runs bootstrap. See [terraform/envs/aws4403/README.md](../terraform/envs/aws4403/README.md) and [CHANGELOG.md](CHANGELOG.md#1725---2026-07-18). |
...
2. **Store credentials securely**
   ...
   - Prefer:
     ...
     - **Config file**: Use `config/app/config.json` with file permissions restricted, storing secrets as encrypted pointers — `{ "_cfgenc": "v1$..." }` locally/Docker or `{ "_sm": "OSCAL/..." }` on EC2 (AWS Secrets Manager). See [Sensitive settings and _cfgenc (local/Docker) or AWS SM (EC2)](#sensitive-settings-and-_cfgenc-localdocker-or-aws-sm-ec2).
...
- [Sensitive settings and _cfgenc (local/Docker) or AWS SM (EC2)](#sensitive-settings-and-_cfgenc-localdocker-or-aws-sm-ec2) – Secret storage for production.
...
- **One-time migration:** If config still has plaintext or legacy `_pass` pointers, deploy runs `backend/scripts/migrate-config-to-sm.mjs`.
- **Local / Docker:** Default `OSCAL_SECRETS_MODE=config` — secrets stored as **`{ "_cfgenc": "v1$..." }`** in `config.json` (PBKDF2 + AES-256-GCM via `backend/utils/configFieldCrypto.js`). Set **`OSCAL_CONFIG_FIELD_SECRET`** or **`SESSION_SECRET`**; Docker entrypoint auto-generates persisted keys under `/data/`. Run **`backend/scripts/migrate-config-to-cfgenc.mjs`** to encrypt legacy plaintext/`_pass`. **Pass vault is optional**: set `OSCAL_PASS_DISABLED=1` in Docker.
...
- **Pass vault on instances:**  
  Compare `config.json` `_sm` (or legacy `_pass`) references with the SM bundle in the AWS console.
...
- Node.js 24 is installed by the deploy script if not present (e.g. via NodeSource).
