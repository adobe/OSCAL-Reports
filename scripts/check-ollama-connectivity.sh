...
echo "  - On the Ollama instance: ensure Ollama listens on 0.0.0.0 (run: ./scripts/run-install-ollama-on-instance.sh then ./scripts/fix-ollama-listen-address.sh). If firewalld is active, open port 11434: sudo firewall-cmd --permanent --add-port=11434/tcp; sudo firewall-cmd --reload"
