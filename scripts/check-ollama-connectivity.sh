...
echo "Next steps:"
echo "  - Connection refused from NLB IPs usually means no healthy targets. In AWS Console: EC2 -> Target Groups -> *-ollama-11434 -> Targets. Ensure Ollama ASG has at least one instance and status is Healthy."
echo "  - If the ASG is at 0, start instances (e.g. set desired capacity to 1 or trigger the Lambda wake). Wait for targets to become healthy, then retry."
echo "  - If firewalld were blocking outbound: sudo firewall-cmd --add-rich-rule='rule family=ipv4 direction=out destination port port=11434 protocol=tcp accept' --permanent; sudo firewall-cmd --reload"
