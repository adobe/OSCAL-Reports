...
echo ""
echo "Tip: ALB health checks use /health/ready (SPA + config + secrets). Run post-deploy smoke via SSM or:"
echo "  ./scripts/ci/post-deploy-smoke.sh ssh:<instance-ip> 3020"
echo "Weighted listeners skip unhealthy targets; host rules and stickiness (see terraform/alb.tf) can change which color you see."
