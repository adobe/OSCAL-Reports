#!/bin/bash
# Upgrade Green Deployment with Data Backup and Restore
...
BACKUP_DIR="$HOME/oscal-green-backup-$BACKUP_DATE"
...
print_header "Green Deployment Upgrade (v1.6.2 → v1.6.5+)"

echo "This script will:"
echo "  1. Backup your current Green deployment data"
echo "  2. Upgrade to version with volume persistence"
echo "  3. Restore your users and configuration"
echo ""
echo "⚠️  WARNING: This will cause downtime for Green deployment"
echo ""
...
Next Steps:
1. Run build_on_truenas.sh in Green directory
...
print_header "Step 2: Upgrading Green Deployment"

echo "Now you need to run the build script from the Green directory."
echo ""
echo "IMPORTANT: Make sure you're in the Green deployment directory!"
echo ""
read -p "Enter the full path to your Green directory: " GREEN_DIR

if [ ! -d "$GREEN_DIR" ]; then
  print_error "Directory not found: $GREEN_DIR"
  exit 1
fi

if [ ! -f "$GREEN_DIR/build_on_truenas.sh" ]; then
  print_error "build_on_truenas.sh not found in $GREEN_DIR"
  exit 1
fi

print_info "Found build script in $GREEN_DIR"
echo ""
...
print_header "✅ Green Deployment Upgrade Complete!"
...
echo ""
echo "📖 Next Steps:"
echo "   1. Test Green deployment: http://YOUR_SERVER:$CONTAINER_PORT"
echo "   2. Verify users and configuration"
echo "   3. Consider upgrading Blue deployment"
echo ""
